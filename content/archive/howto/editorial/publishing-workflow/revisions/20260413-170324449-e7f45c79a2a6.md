...
{% hero eyebrow="Guide" title="How to Publish a Page Without Breaking the Site." deck="The publishing flow is short: write the draft, run the check step, accept the page, and let the runtime rebuild the live site." aside="The goal is not more process. The goal is one clear path that humans and AI tools can both follow." actionHref="/how-it-works" actionLabel="How It Works" /%}

{% sectionCopy eyebrow="Write the Draft" title="Start with One Markdown File." %}
Write the page in the submit-here directory so drafts stay separate from published history and generated site data. That keeps the working surface obvious for people and AI tools alike.
...
Published source is mirrored into the archive and the live site data is rebuilt from there.
